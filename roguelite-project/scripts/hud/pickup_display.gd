...
@export var pickup_display_template : PackedScene

@onready var grid_container : GridContainer = $GridContainer

var displays : Array[PickupCountDisplay] 
var player_inventory : Inventory


func _ready():
	#Finding and connecting the players inventory to this file
...
func _on_player_inventory_resource_count_changed(type : ResourceItem, new_count : int) -> void:
	...
	var current_display : PickupCountDisplay
	...
	for display in displays:
		if(display.resource_type == type):
			...
			break
	#If none exist create a new pickup counter
	if(current_display == null):
		var new_display : PickupCountDisplay = pickup_display_template.instantiate() as PickupCountDisplay
